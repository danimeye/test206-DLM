...
def get_user_tweets(user):
	...
	if unique_identifier in CACHE_DICTION: 
		...
		twitter_results = api.user_timeline(user) # get it from the internet
...
for tweet in umich_tweets:
	s1 = 'INSERT OR IGNORE INTO Tweets Values (?, ?, ?, ?, ?)'
	tweet_vals = (tweet['id_str'],tweet['text'],tweet['user']['id'], tweet['user']['created_at'], tweet['retweet_count'])
	cur.execute(s1, tweet_vals)
